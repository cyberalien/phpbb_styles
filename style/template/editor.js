...
function colorPalette(dir, width, height)
{
    ...
    if (!item)
    {
    	item = document.getElementById('colour_palette');
    	if (item)
    	{
			item = item.getElementsByTagName('dd');
			item = (item.length) ? item[0] : false;
		}
		if (!item)
		{
			onload_functions.push("colorPalette('" + dir + "', " + width + ", " + height + ");");
			return;
		}
	}
